...
const placeOrder = async (req, res) => {
    try {
        const { products, shipping, totalAmount, userId } = req.body;
        ...
        const populatedProducts = await Promise.all(products.map(async (item) => {
            ...
            if (!productData) throw new Error("Product not found: " + item.product_id._id);
...
const getOrders = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: "Orders fetching faile", detail: err.message });
    }
}
