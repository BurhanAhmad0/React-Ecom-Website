...
const Cart = () => {
  ...
  const { cart, handleUpdateQuantity, handleProductDelete, CartLoading } =
    useApp(); // Get the actual cart state and loading status

  let shippingCost = 5.0;
  const totalAmount = cart
    ? cart.reduce(
        (acc, item) =>
          acc + (item.product_id?.product_price || 0) * item.quantity, // Safely access price
        0
      )
    : 0;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          {CartLoading ? (
            ...
            cart.map((item, index) => (
              <div
                key={index} // Use product_id._id for key, fallback to item._id
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-600">
                    $
                    {(parseFloat(item.product_id?.product_price) || 0).toFixed(
                      2
                    )}
                  </p>
                  ...
                  <div className="flex items-center mt-2">
                    ...
                    <button
                      ...
                      } // You'd connect this to your update function
                ...
                <button
                  onClick={() => handleProductDelete(item.product_id?._id)}
                  className="cursor-pointer text-red-600 hover:underline mt-4 md:mt-0"
                >
                  Remove
                </button>
              </div>
