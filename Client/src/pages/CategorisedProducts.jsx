    ...
    const fetchCategorisedProducts = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_BASE}/product/category/${category}`,
          {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
          {categorisedProducts.map((product) => (
            <div
              key={product._id}
              className="bg-white p-6 rounded-lg shadow hover:shadow-md transition-shadow duration-300 flex flex-col"
            >
              <img
                loading="lazy"
              ...
              <button
                ...
                className="cursor-pointer mt-auto bg-blue-600 text-white py-2 rounded-md hover:bg-blue-700 transition-colors"
              >
                Add to Cart
              </button>
            </div>
