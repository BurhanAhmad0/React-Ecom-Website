...
import { useApp } from "../Context/AppContext.jsx"; // Assuming AppContext is in 'src/context'
import { useAuth } from "../Context/AuthContext.jsx"; // Assuming AuthContext is in 'src/context'
...
const checkoutSchema = yup.object().shape({
  shipping: yup.object().shape({
    ...
    address2: yup
      .string()
      .max(200, "Address cannot exceed 200 characters")
      .nullable(), // .nullable() allows null or undefined, .default('') can set a default empty string
    city: yup
      .string()
    ...
    zipCode: yup
      ...
      .max(20, "Invalid zip code")
      .matches(/^[0-9A-Za-z\s-]{3,}$/, "Invalid zip/postal code format"), // More generic for international
    country: yup
      .string()
    ...
    phone: yup
      ...
      .matches(/^\+?[0-9\s()-]*$/, "Invalid phone number format"), // Allow some common phone chars
  }),
});
...
const CheckoutPage = () => {
  ...
  const subtotal =
    cart?.reduce(
      (acc, item) =>
        acc + (item.product_id?.product_price || 0) * item.quantity,
      0
    ) || 0;
  const shippingCost = subtotal > 0 ? 5.0 : 0; // Example: free shipping if cart empty, otherwise $5
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_BASE}/order`,
        payload,
        {
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              ...
              <div>
                <label
                  htmlFor="shipping.stateProvince"
                  className="block text-sm font-medium text-gray-700 mb-1"
                >
                  State / Province
                </label>
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                {errors.shipping?.stateProvince && (
                  <p className="mt-1 text-sm text-red-500">
                    {errors.shipping.stateProvince.message}
                  </p>
                )}
              </div>
              <div>
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-lg border border-gray-100 h-fit sticky top-8">
          ...
          {cart?.map((item) => (
            <div
              ...
              className="flex items-center justify-between mb-4 pb-2 border-b border-gray-100 last:border-b-0 last:pb-0"
            >
              ...
              <span className="font-medium text-gray-800">
                $
                {(
                  (item.product_id?.product_price || 0) * item.quantity
                ).toFixed(2)}
              </span>
            </div>
          ))}
