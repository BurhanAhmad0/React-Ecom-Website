  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await axios.put(
        `${import.meta.env.VITE_API_BASE}/user/${user.userId}`,
        data,
        {
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-md p-6 sm:p-10">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
          ...
          {loading ? (
            ...
            <div
              onMouseEnter={() => setUplaodImageHover(true)}
              onMouseLeave={() => setUplaodImageHover(false)}
              className="relative w-32 h-32 rounded-full bg-blue-200 overflow-hidden p-7"
            >
              <img
                loading="lazy"
                src={userIcon}
              ...
              <div
                className={`overlay cursor-pointer w-full h-full rounded-full bg-black/25 absolute top-0 left-0 z-10 ${
                  UplaodImageHover ? "flex" : "hidden"
